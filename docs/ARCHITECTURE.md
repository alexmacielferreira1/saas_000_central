...
`User`, `Tenant` e `Membership` existem, porém a seleção explícita de tenant, isolamento completo em todos os repositórios, permissões granulares e auditoria persistente ainda pertencem à C1. Os papéis atuais são uma proteção transitória e não substituem o RBAC/ABAC planejado.
