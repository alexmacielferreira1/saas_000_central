...
Para entregar o trabalho a outro chat/agente, começar por `docs/CENTRAL_MASTER_IMPLEMENTATION_CHECKLIST.md` e `docs/CENTRAL_SCREEN_IMPLEMENTATION_MATRIX.md`. A matriz transforma o planejamento em telas/rotas e exige código funcional por sessão. O índice completo está em `docs/README.md`, o plano técnico em `docs/CENTRAL_IMPLEMENTATION_PLAN.md`, e o inventário conceitual em `docs/CENTRAL_CAPABILITY_CATALOG.md`. O README dentro de `frontend/` pertence ao export histórico e não é instrução de publicação da reconstrução.
