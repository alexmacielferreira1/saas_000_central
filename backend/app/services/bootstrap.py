...
def bootstrap_initial_admin(session: Session, settings) -> BootstrapResult:
    if not settings.bootstrap_admin_email or not settings.bootstrap_admin_password or not settings.bootstrap_admin_name:
    ...
    if existing:
        ...
        if any(m.role == "superadmin" and t.slug == settings.bootstrap_tenant_slug for m, t in memberships):
    ...
    user = User(email=email, email_normalized=email, password_hash=hash_password(settings.bootstrap_admin_password.get_secret_value()), full_name=settings.bootstrap_admin_name)
    session.add(user); session.flush()
    session.add(Membership(user_id=user.id, tenant_id=tenant.id, role="superadmin")); session.commit()
