...
def payload(session: Session, auth_session) -> SessionResponse:
    ...
    return SessionResponse(
        ...
        memberships=[
            MembershipResponse(tenant_id=m.tenant_id, tenant_name=t.name, role=m.role)
            for m, t in memberships
        ],
        selected_tenant_id=auth_session.selected_tenant_id,
    )
...
def logout(
    ...
    if token:
        ...
        if stored and stored.revoked_at is None:
            revoke_auth_session(session, stored, datetime.now(UTC))
    settings = get_settings()
...
def google_start():
    ...
    if not settings.google_client_id or not settings.google_client_secret:
        return RedirectResponse(f"{settings.frontend_url}/login?google_error=not_configured")
...
async def google_callback(
    ...
    state: str | None = None,
    error: str | None = None,
    oauth_state: Annotated[str | None, Cookie(alias=GOOGLE_STATE_COOKIE)] = None,
):
    settings = get_settings()
    ...
    if not oauth_state or not state or not compare_digest(state, oauth_state):
        return failed("invalid_state")
    if error:
        return failed("access_denied" if error == "access_denied" else "provider_failed")
    if not settings.google_client_id or not settings.google_client_secret:
