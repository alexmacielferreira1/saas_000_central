...
def test_bootstrap_creates_superadmin_once_without_resetting_it():
    engine = create_engine("sqlite+pysqlite:///:memory:", poolclass=StaticPool); Base.metadata.create_all(engine)
    factory = sessionmaker(bind=engine, expire_on_commit=False)
    settings = SimpleNamespace(bootstrap_admin_email="alexmacielferreira@gmail.com", bootstrap_admin_password=SimpleNamespace(get_secret_value=lambda: "senha-segura"), bootstrap_admin_name="Alex Maciel Ferreira", bootstrap_tenant_name="Central SaaS", bootstrap_tenant_slug="central")
