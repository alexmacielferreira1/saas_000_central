...
def auth_client():
    ...
    with factory() as session:
        user = User(
            ...
            email_normalized="alexmacielferreira@gmail.com",
            password_hash=hash_password(PASSWORD),
            full_name="Alex Maciel Ferreira",
        )
...
def test_login_sets_http_only_cookie_and_restores_session(auth_client):
    response = auth_client.post(
        "/api/v1/auth/login",
        json={"email": "  AlexMacielFerreira@GMAIL.COM ", "password": PASSWORD},
    )
...
def test_invalid_login_has_uniform_public_error(auth_client):
    response = auth_client.post(
        "/api/v1/auth/login", json={"email": "missing@example.com", "password": PASSWORD}
    )
...
def test_logout_revokes_session(auth_client):
    auth_client.post(
        "/api/v1/auth/login", json={"email": "alexmacielferreira@gmail.com", "password": PASSWORD}
    )
...
def google_settings():
    return SimpleNamespace(
        ...
        google_client_secret=SimpleNamespace(get_secret_value=lambda: "secret"),
        google_redirect_uri="http://127.0.0.1:8011/api/v1/auth/google/callback",
...
def test_google_start_reports_missing_configuration(auth_client):
    response = auth_client.get("/api/v1/auth/google/start", follow_redirects=False)
    assert response.status_code == 307
    assert response.headers["location"].endswith("/login?google_error=not_configured")
...
def test_google_callback_authenticates_only_existing_verified_account(auth_client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.auth.exchange_google_code",
        AsyncMock(return_value={"email": "alexmacielferreira@gmail.com", "email_verified": True}),
    )
